...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$needle = $env:ROOT.TrimEnd('\'); Get-CimInstance Win32_Process | Where-Object { (($_.Name -in @('python.exe','pythonw.exe')) -and ($_.CommandLine -like ('*' + $needle + '*'))) -or (($_.Name -eq 'powershell.exe') -and ($_.CommandLine -like '*wait_for_live_preview_and_restart_obs.ps1*')) } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"
