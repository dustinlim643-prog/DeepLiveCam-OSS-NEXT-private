...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Add-Content -LiteralPath ($env:ROOT + 'logs\operation_log.txt') -Encoding UTF8 -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' start UI + OBS requested')"
...
start "DeepLiveCam UI" "%ROOT%python\python.exe" "%ROOT%run.py" --execution-provider cuda --execution-threads 2 --frame-processor face_swapper_hyperswap --live-resizable --live-fps-debug --similar-face-distance 1.5 --live-face-smooth 0.35 --live-face-fit-scale 1.06 --quality-preset balanced -l zh
...
if exist "%OBS_EXE%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "Add-Content -LiteralPath ($env:ROOT + 'logs\operation_log.txt') -Encoding UTF8 -Value ((Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' OBS started for Live Preview capture')"
) else (
